...
CATEGORICAL_COLUMNS = ['aisle_id', 'department_id',]
...
CROSS_FEATURE_CONFIG = pd.DataFrame(
        ...
        'key_list':
            [['user_id', 'aisle_id'], ['user_id', 'department_id'], 
        ...
        'f':
            [lambda d, k: d[k[0]] * 150 + d[k[1]], lambda d, k: d[k[0]] * 25 + d[k[1]],
             lambda d, k: d[k[0]] * 10 + d[k[1]], lambda d, k: d[k[0]] * 35 + d[k[1]], lambda d, k: d[k[0]] * 25 + d[k[1]],
             lambda d, k: d[k[0]] * 10 + d[k[1]], lambda d, k: d[k[0]] * 35 + d[k[1]], lambda d, k: d[k[0]] * 25 + d[k[1]],
             lambda d, k: d[k[0]] * 10 + d[k[1]], lambda d, k: d[k[0]] * 35 + d[k[1]], lambda d, k: d[k[0]] * 25 + d[k[1]],
             lambda d, k: d[k[0]] * 10 + d[k[1]], lambda d, k: d[k[0]] * 35 + d[k[1]], lambda d, k: d[k[0]] * 25 + d[k[1]]]})
if 'cross_key' in CROSS_FEATURE_CONFIG:
    for cross_key in CROSS_FEATURE_CONFIG['cross_key']:
...
def create_model(classes):
    model = Sequential()
    model.add(Dense(hidden_units[0], activation='sigmoid', input_dim = len(dnn_features)))
    if dropout_rate > 0:
...
def keras_train(nfolds = 10):
    ...
    for train_index, test_index in kf:
       ...
       # pd.DataFrame(Y_valid).to_csv("valid_labels", index = False)
        ...
        model.fit(X_train, Y_train, batch_size=batch_size, epochs=dnn_epoch,
