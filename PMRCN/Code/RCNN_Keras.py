...
def Create_RCNN(max_token, embedding_dim, num_classes, context_vector_dim, hidden_dim):
    """
    """
    ...
    MAX_TOKENS = max_token# word2vec.syn0.shape[0]
...
def gen_RCNN_Input(input_text, max_num_words, max_len):
    """
    """
    ...
    left_context_as_array = np.array([[max_num_words] + list(tokens[:-1]) for tokens in output_sequences])
    # We shift the document to the left to obtain the right-side contexts.
    right_context_as_array = np.array([list(tokens[1:]) + [max_num_words] for tokens in output_sequences])
